...
class Player:
    def __init__(self, 
        ...
        self.barriers = [[0, 2], [1,2], [3, 2], [4, 2]]    # Cells above the two barriers
        self.starting_positions = [[0, 0], [4, 0]]
    ...
    def move(self, movement):
        ...
        if movement == 'left'  and 'left' in self.movements:
            ...
            if new_position in self.barriers and self.position in self.starting_positions:
                if random.uniform(0, 1) < 0.5:      # 50% chance to go through the barrier
...
class Grid:
    ...
    def compute_reward(self,
        ...
        '''
        if old_state == [0, 0] and new_state == [0, 0] and movement == 'up':
            return -1
        if old_state == [2, 0] and new_state == [2, 0] and movement == 'up':
            return  -1
        if old_state == [0, 1] and new_state == [0, 1] and movement == 'down':
            return -1
        if old_state == [2, 1] and new_state == [2, 1] and movement == 'down':
            return -1
        else:
